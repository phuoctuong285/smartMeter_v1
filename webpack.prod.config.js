...
var ExtractCSS = new ExtractTextPlugin('main-css.css')
var ExtractSCSS = new ExtractTextPlugin('main-scss.css')
...
module.exports = {
  ...
  module: {
    loaders: [{
      ...
      loader: ExtractCSS.extract('style', 'css?importLoaders=1&-raw!postcss')
    }, {
      test: /\.css$/,
      exclude: [/\/onsen-css-components.css$/, path.join(__dirname, 'src')],
      loader: ExtractSCSS.extract('style', 'css?sourceMap')
    },
    {
      test: /\.scss$/,
      include:[path.join(__dirname,'src','public','scss')],
